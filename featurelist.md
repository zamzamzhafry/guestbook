...
1. Project Setup
   Set up Next.js application with the necessary dependencies.
   Install packages for QR code generation (qrcode.react or similar), database connection (Prisma, if using), and other libraries (like axios for API calls).
   Initialize a database with Prisma for guest management (e.g., PostgreSQL or MongoDB).
2. Database Schema and Backend
   Define a database schema for guests with fields like name, attendance status, invitation details, etc.
   Add schema for user roles (admin, front desk/keeper, etc.) if needed.
   Set up Prisma migrations.
   Create basic API routes (e.g., /api/guests, /api/attendance) to handle CRUD operations for guests.
3. User Roles and Authentication
   Implement authentication for different user roles (e.g., admin, front desk).
   Set up access control for pages based on user roles.
...
5. Additional Components and Functionality
